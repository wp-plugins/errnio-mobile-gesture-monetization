...
function errnio_load_script()
{
	if(get_option('errnio_api')){
		$script_url = "http://service.errnio.com/script/".get_option('errnio_api');
		
		wp_register_script( 'errnio_script', $script_url, array( 'jquery' ), '1.0', false );
		wp_enqueue_script( 'errnio_script');
	}
	else{
		$script_url = "http://service.errnio.com/script/54982f10154585edd75615e0";
...
function wptuts_admin_notices() {
	if(!get_option('errnio_api')){
		echo( '<div class="error"> <p> Errnio needs to be configured, enter your code <a href="'.admin_url( 'options-general.php?page=errnio.php' ).'">here</a> </p> </div>');
	}
}
